...
export default function PortfolioGrid() {
  ...
  const getCategoryFromLanguage = (language, repoName) => {
    ...
    const lang = language || ''
    if (name.includes('next') || name.includes('fullstack')) return 'Full Stack'
    if (name.includes('react') || lang === 'JSX') return 'React'
    if (name.includes('landing') || name.includes('page')) return 'Landing Pages'
  ...
  const getImageForProject = (projectName) => {
    ...
    if (name.includes('quiz')) return "https://images.unsplash.com/photo-1606326608606-aa0b62935f2b?w=600&h=400&fit=crop"
    if (name.includes('weather')) return "https://images.unsplash.com/photo-1592210454359-9043f067919b?w=600&h=400&fit=crop"
    if (name.includes('portfolio')) return "https://images.unsplash.com/photo-1545235617-9465d2a55698?w=600&h=400&fit=crop"
  ...
  useEffect(() => {
    const fetchGitHubRepos = async () => {
      try {
        ...
        // ✅ Filter coming soon using state, not window
        const filteredComingSoon = allComingSoonProjects.filter(
          project => !repoNameSet.has(project.githubIdentifier.toLowerCase())
        )
        setComingSoonProjects(filteredComingSoon)
        ...
        const transformedProjects = repos.map(repo => ({
          ...
          category: getCategoryFromLanguage(repo.language, repo.name),
        ...
        setGithubProjects([])
        setComingSoonProjects(allComingSoonProjects) // ✅ fallback via state
      } finally {
        setLoading(false)
      }
    }
  ...
  const getFilteredProjects = () => {
    ...
    return allProjects.filter(p => p.category === activeCategory && p.status !== 'coming')
  }
  ...
  const handleProjectClick = (project) => {
    if (project.status === 'live' && project.githubUrl) {
      window.open(project.githubUrl, '_blank') // ✅ fine - only runs on click (client)
    } else {
            ...
            {filteredProjects.map((project, idx) => (
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mb-3 text-sm line-clamp-2">{project.description}</p>
